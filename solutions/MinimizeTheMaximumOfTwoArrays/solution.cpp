// https://leetcode.com/problems/minimize-the-maximum-of-two-arrays/discuss/2947014/Formula
// the above doesn't use bs but a formula


// https://leetcode.com/problems/minimize-the-maximum-of-two-arrays/discuss/2946511/Binary-Search-%2B-Venn-Diagram-(Set-Diagrams)-or-Explanation-with-diagram
...
public:
    ...
    bool check(long long d1, long long d2, long long u1, long long u2, long long x) {
        ...
        */
        ...
        // below my solution
        /*if (u1 > A_ || u2 > B_) return false;
        long long rest = A_ - u1; // having filled 1st array, we have this many left to use
        long long for2ndArray = B_ - A_I_B_; // this many we have to use for 2nd array
        if (rest > A_I_B_) for2ndArray += A_I_B_;
        else               for2ndArray += rest;
        return for2ndArray >= u2;*/
    }
};
